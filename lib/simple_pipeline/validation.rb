  ...
  module Validation
    ...
    module ClassMethods
      def validate(validation_lambda)
        ...
        class_variable_get(:@@validations) << validation_lambda
      end
    end
    ...
    def validate(payload)
      self.class.class_variable_get(:@@validations).each do |validation_lambda|
        begin
            raise SimplePipeline::Validation::Error, "#{self.class} - #{payload}" unless validation_lambda.call(payload)
          rescue
            raise SimplePipeline::Validation::Error, "#{self.class} - #{payload}"
          end
      end
    end
  end
